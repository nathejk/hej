...
// (delta 0), on time with an elapsed leg time (a `relative` post), late (positive delta), early
// (negative delta), a `none`-scheme post with no verdict, an unattributable registration, and a
// bandit catch that never carries one. A verdict here is a literal, not computed — the mock stands in
// for the projection, and the projection is where verdictFor runs.
//
...
func NewMockSource() Source {
	...
	m := &mockSource{byPatrol: map[string][]Scan{
		users.MockSpejderPatrolID: {
			...
			// A `relative` post: on time, and the leg took 45 minutes. The elapsed time is only meaningful
			// here — a relative window is measured from the patrol's own arrival at the previous line.
			{ID: "scan-1042-3", Kind: KindCheckpoint, Label: "Post 3 – Ans Bro", CheckpointID: "cp-3", Lat: pos(56.2311), Lng: pos(9.5216), ScannedAt: evening(21, 14), Verdict: &Verdict{OnTime: true, DeltaSeconds: 0, SpentSeconds: mins(45)}},
			...
			// Early: arrived before the post opened. Signed delta keeps this distinguishable from late.
			{ID: "scan-1042-7", Kind: KindCheckpoint, Label: "Post 5 – Tange Sø", CheckpointID: "cp-5", Lat: pos(56.2450), Lng: pos(9.5900), ScannedAt: evening(22, 55), Verdict: &Verdict{OnTime: false, DeltaSeconds: -5 * 60}},
...
// mins renders a duration in whole minutes as the seconds the verdict carries.
func mins(m int) *int { s := m * 60; return &s }
