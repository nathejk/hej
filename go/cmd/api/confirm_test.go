...
func TestReportIncorrect_PublishesReasonedReport(t *testing.T) {
	for _, reason := range []string{"wrong", "unknown"} {
		t.Run(reason, func(t *testing.T) {
			...
			cookies := authedCookies(t, app, srv, confirmPhone, confirmNormalized)
			resp := postJSONWithCookies(t, srv.URL+"/api/me/profile/report-incorrect",
				`{"reason":"`+reason+`"}`, cookies)
			io.Copy(io.Discard, resp.Body)
			resp.Body.Close()
			if resp.StatusCode != http.StatusNoContent {
				t.Fatalf("status = %d, want 204", resp.StatusCode)
			...
			var body person.GuardianReported
			if err := pub.Messages[0].Body(&body); err != nil {
				t.Fatalf("decode body: %v", err)
			}
			// The two reasons are different jobs for an organizer — fix vs check — so the
			// distinction has to survive to the log.
			if body.Reason != reason {
				t.Errorf("reason = %q, want %q", body.Reason, reason)
			}
			if body.PersonID != "mock-spejder-1" {
				t.Errorf("personId = %q, want the session's user", body.PersonID)
			}
...
// An unknown reason is refused rather than coerced: silently storing it as "wrong" would
// tell an organizer to correct a number the member never claimed was incorrect.
func TestReportIncorrect_RejectsUnknownReason(t *testing.T) {
	pub := &cqrstest.Publisher{}
	app := confirmTestApp(t, pub, unverifiedSpejder())
	srv := httptest.NewServer(app.routes())
	defer srv.Close()
	...
	if len(pub.Messages) != 0 {
		t.Errorf("published %d events for an unknown reason, want 0", len(pub.Messages))
	}
}

// The report path is reachable regardless of whether confirmation is "required": a member
// whose number was already verified may still discover it is wrong.
func TestReportIncorrect_WorksForAVerifiedMember(t *testing.T) {
	guardian := "+4520000001"
	at := time.Date(2026, 8, 30, 12, 0, 0, 0, time.UTC)
	pub := &cqrstest.Publisher{}
...
func TestReportIncorrect_RequiresAuth(t *testing.T) {
	app := confirmTestApp(t, &cqrstest.Publisher{}, unverifiedSpejder())
	srv := httptest.NewServer(app.routes())
	defer srv.Close()

	resp := postJSON(t, srv.URL+"/api/me/profile/report-incorrect", `{"reason":"wrong"}`)
	io.Copy(io.Discard, resp.Body)
	resp.Body.Close()
	if resp.StatusCode != http.StatusUnauthorized {
		t.Fatalf("status = %d, want 401", resp.StatusCode)
