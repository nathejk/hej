...
- 2026-07-30 15:45 — Fix (post-completion): `ui-dev` stage was missing `EXPOSE 80`, so Traefik's docker provider couldn't discover the service's port. Added `EXPOSE 80` (Vite dev server listens on :80; default port so no loadbalancer label needed). Rebuilt `ui-dev`; `docker image inspect` confirms `ExposedPorts: {"80/tcp":{}}`.
- 2026-07-30 15:55 — Fix (post-completion): prod image now serves HTTP on the default port **80** (was 4000). Set `ENV PORT=80` + `EXPOSE 80` on the `prod` stage so it works with Traefik's default port discovery on one origin. `PORT` env still overrides. Dev `api` keeps `PORT=4000` (internal, via Vite proxy); the Go config default remains 4000, overridden per-image via env.
