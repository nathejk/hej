...
//
// `defaultEnv()` yields a **synthesised** environment when the dev layer has registered one,
// so the app can be walked on a laptop. Three things about how that is done are deliberate:
//
...
// - **It arrives by registration, not by import.** This module deliberately does not import
//   `@/dev/*`. A static import would put the whole dev layer — its fake user-agent strings
//   included — into the production bundle, because `import.meta.env` guards *hide* code, they
//   do not remove code that something still imports. That was measured, not assumed: an
//   earlier cut of this had the UA table sitting in `dist/assets/index-*.js`. The dev
//   bootstrap dynamic-imports itself from `main.ts` behind `import.meta.env.DEV` and calls
//   `setDevEnvProvider`, so in production the branch, the chunk and the strings are all gone.
// - **It does not violate constraint 1 or 2 above.** The provider is a plain synchronous
//   function and the registration happens before the router's first navigation, so the guard
//   still decides synchronously with no awaited work and no Pinia dependency.
...
// ---------------------------------------------------------------------------
// The dev simulation hook (PRD 014). See the header for why this is a registration point
// rather than an import of `@/dev/*`.
// ---------------------------------------------------------------------------

/** Supplies a synthesised environment, or `null` to use the real one. */
export type DevEnvProvider = () => PlatformEnv | null

let devEnvProvider: DevEnvProvider | null = null

/**
 * Registers the dev layer's environment provider. Called once from `@/dev/bootstrap`, which
 * `main.ts` only imports under `import.meta.env.DEV`.
 *
 * The `DEV` guard here is belt-and-braces: the only caller is already dev-only, but a stray
 * future call from product code would otherwise be a way to simulate a device in production.
 */
export function setDevEnvProvider(provider: DevEnvProvider | null) {
  if (!import.meta.env.DEV) return
  devEnvProvider = provider
}

function devEnv(): PlatformEnv | null {
  // Folded to `false` at build time, so everything below — and the provider itself, which
  // nothing else references — drops out of a production bundle.
  if (!import.meta.env.DEV) return null
  return devEnvProvider?.() ?? null
}

/**
 * The simulated navigator, or `null` when nothing is being simulated.
 *
